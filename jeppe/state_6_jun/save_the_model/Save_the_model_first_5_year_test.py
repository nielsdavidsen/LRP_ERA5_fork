...
##################################T2m#############################################
    ...
    def prepare_data_for_tensorflow(self, test_size = .1 , print_shapes = True):
        ## make the mask the first 1827 in first dimension
    ...
    # Split the data using the mask
        ...
        y_test = scaler.fit_transform(self.target[:1827].reshape(-1, 1))
        y_train =scaler.transform(self.target[1827:].reshape(-1, 1))
        if print_shapes:
    ...
    def train_model(self,loss_fcn = 'mae', epochs=1000, batch_size=128, learning_rate=1e-4, validation_split=0.2,
                      weight_decay=1e-5, patience=5, factor=0.5, early_stopping_patience = 12, model_save_name = None):
