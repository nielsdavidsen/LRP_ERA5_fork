...
class Model_FFN:
    ...
    def plot_model_on_test(self, title = 'Model Performance on Test Data', save_name = None):
        ...
        fig, ax = plt.subplot(nrows=2, ncols=1, figsize=(10, 5))
        ...
        ax[1].hist(residuals, label='Residuals (truth - pred.)', bins=50, color='forestgreen', alpha=0.7, histtype='stepfilled', edgecolor='black')
        ...
        for a in ax:
            a.grid(True, linestyle='--', color='white')
            a.set_facecolor('gainsboro')
            a.legend(loc='lower left', fintsize=12)
