...
class Model_FFN:
    ...
    def lrp_calc_and_plot(self, test_shape=None, save_name = None):
        from captum.attr import LRP
        import cartopy.crs as ccrs
        import cartopy.feature as cfeature
    
        input_tensor = torch.from_numpy(self.X_test).view(self.X_test.shape[0], -1).clone().detach().requires_grad_(True)
        ...
        if test_shape is not None:
            attr_sum = np.sum(attributions.reshape(test_shape), axis=0)
        ...
        ax[0].set_title('LRP Attribution for 850hPa Temperature')
        cbar = ax[0].pcolormesh(self.longitude_range,
                                self.latitude_range,
                                lrp_norm[0],
                                cmap='coolwarm',
        ...
        ax[1].set_title('LRP Attribution for Mean Sea Level Pressure')
        ax[1].pcolormesh(self.longitude_range,
                                self.latitude_range,
                                lrp_norm[1],
                                cmap='coolwarm',
                                transform=ccrs.PlateCarree()
